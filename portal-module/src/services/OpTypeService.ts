...
export const createOperationType = async (opTypeData: {
    ...
    try {
        // Wrap opTypeData within `operationTypeDTO` and convert specializations to objects
        const formattedData = {
            operationTypeDTO: {
                OperationTypeName: opTypeData.operationTypeName,
                OperationTime: opTypeData.operationTime,
                Specializations: opTypeData.specializations.map((name) => ({ Name: name })),
            },
        };
...
export const updateOperationType = async (opTypeData: {
    ...
    try {
        const response = await fetch(`${API_URL}/Update`, {
            method: 'PUT',
            headers: getHeaders(),
            body: JSON.stringify(opTypeData),
        });

        console.log("Update Operation Type Response Status:", response.status);
        if (!response.ok) throw new Error('Failed to update Operation Type');
        ...
        console.error("Error in updateOperationType:", error);
        throw error;
    }
};

export const deleteOperationType = async (operationTypeName: string) => {
    const url = `${API_URL}/operationType?operationTypeName=${encodeURIComponent(operationTypeName)}`;
    ...
    try {
        ...
        const data = await response.json();
        console.log("Delete Operation Type Response Data:", data);
